...
def render(app:Dash, values:Values)->html.Div:
    ...
    def update_subplots(clickData_choropleth:dict, clickData_suplots:dict, figure_before:dict)->dict:
        ...
        data_frames = [df[df["iso_code"]==country][["year","co2"]].rename(columns={"co2":str(country)}) for country in iso_code]
        print(data_frames)

        histo_data = reduce(lambda left, right: pd.merge(left, right, on="year", how="outer"), data_frames)
